...
# 뉴스 클러스터링
# 여러 언론사에서 쏟아지는 뉴스,
# 특히 속보성 뉴스를 보면 비슷비슷한 제목의 기사가 많아 정작 필요한 기사를 찾기가 어렵다.
...
def solution(str1, str2):
    ...
    sum2 = 0
    for x in inter:
        count_x = min(list1.count(x), list2.count(x))
        sum1 += count_x
    for y in uni:
        count_y = max(list1.count(y), list2.count(y))
        sum2 += count_y
...
def solution(str1, str2):
    ...
    inter = set(str1).intersection(set(str2))
    uni = set(str1).union(set(str2))
    # print(inter, uni)

    if not uni:
        return 65536

    return int(65536*(sum(min(str1.count(node), str2.count(node)) for node in inter) / sum(max(str1.count(node), str2.count(node)) for node in uni)))
